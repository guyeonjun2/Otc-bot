...
class UserDetailModal(Modal):
    def __init__(self, bot, carrier):
        super().__init__(title=f"{carrier} 인증 정보 입력")
        self.bot = bot
        self.u_name = TextInput(label="이름", placeholder="실명 입력", min_length=2, max_length=5)
        self.u_phone = TextInput(label="전화번호", placeholder="'-' 제외 숫자만", min_length=10, max_length=11)
        self.u_bank = TextInput(label="은행명", placeholder="입금하실 은행명")
        self.u_account = TextInput(label="계좌번호", placeholder="계좌번호 입력")
...
class CarrierSelectView(View):
    ...
    async def mvno(self, it, b):
        view = View()
        for c in ["SKT 알뜰", "KT 알뜰", "LGU+ 알뜰"]:
            btn = Button(label=c, style=discord.ButtonStyle.secondary)
            async def callback(i, carrier=c): await i.response.send_modal(UserDetailModal(self.bot, carrier))
            btn.callback = callback
            view.add_item(btn)
        await it.response.edit_message(content="알뜰폰 통신사를 선택해주세요.", view=view)
...
class DepositApproveView(View):
    ...
    async def approve(self, it, btn):
        ...
        try:
            u = await self.bot.fetch_user(self.user_id)
            await u.send(f"💰 **충전 완료:** {self.amount:,}원이 지급되었습니다!")
        except: pass
    ...
    async def reject(self, it, btn):
        ...
        await it.response.send_message("❌ 충전 거부 처리됨", ephemeral=True)
        await it.message.delete()
...
class ChargeModal(Modal):
    def __init__(self, bot):
        ...
        self.u_amount = TextInput(label="입금 금액", placeholder="숫자만 입력 (쉼표/원 제외)")
        self.add_item(self.u_sender); self.add_item(self.u_amount)

    async def on_submit(self, interaction):
        await interaction.response.defer(ephemeral=True)
        num_str = "".join(filter(str.isdigit, self.u_amount.value))
        if not num_str: return await interaction.followup.send("❌ 금액은 숫자만 입력해주세요.", ephemeral=True)
        ...
        if log_ch:
            ...
            embed.add_field(name="신청자", value=interaction.user.mention, inline=True)
            embed.add_field(name="입금자", value=sender, inline=True)
...
class OTCView(View):
    ...
    async def check_verify(self, it):
        ...
        await it.response.send_message("🔒 본인인증이 필요합니다.", view=CarrierSelectView(self.bot), ephemeral=True)
        return False

    @discord.ui.button(label="💰 충전", style=discord.ButtonStyle.primary, row=0)
    async def charge(self, it, btn):
    ...
    async def transfer(self, it, btn):
        if await self.check_verify(it):
            await it.response.send_message("📤 **송금 서비스 안내**\n현재 송금 기능은 점검 중입니다. 관리자에게 문의하세요.", ephemeral=True)
    ...
    async def info(self, it, btn):
        ...
        embed.add_field(name="누적 금액", value=f"```{spent:,.0f}원```", inline=True)
        embed.add_field(name="현재 등급", value=f"```{rank_name}```", inline=False)
        await it.response.send_message(embed=embed, ephemeral=True)

    @discord.ui.button(label="❓ 도움말", style=discord.ButtonStyle.secondary, row=1)
    async def help(self, it, btn):
        embed = discord.Embed(title="❓ 도움말 및 이용 안내", color=discord.Color.light_gray())
        embed.description = (
            "**1. 충전 안내**\n'충전' 버튼을 눌러 성함과 금액을 입력 후 입금해주세요.\n\n"
            "**2. 송금 안내**\n코인 주소로 송금을 진행합니다. (현재 점검 중)\n\n"
            "**3. 등급 시스템**\n누적 충전 금액에 따라 자동으로 역할이 부여됩니다.\n\n"
            "**4. 주의 사항**\n타인 명의 입금 시 처리가 지연될 수 있습니다."
        )
        await it.response.send_message(embed=embed, ephemeral=True)
...
@bot.tree.command(name="otc", description="자판기 메인 출력")
