...
class ApproveView(View):
    ...
    async def approve(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            await interaction.message.edit(content=f"✅ <@{self.user_id}> 충전 승인 및 등급 업데이트 완료", embed=None, view=None)
        except Exception as e:
            await interaction.followup.send(f"❌ 승인 실패: {e}", ephemeral=True)
    ...
    async def reject(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.edit_message(content="❌ 요청이 거부되었습니다.", embed=None, view=None)
...
class OTCView(View):
    ...
    async def charge(self, interaction: discord.Interaction, button: Button):
        ...
        amt_input = TextInput(label="금액", placeholder="숫자만 입력 (예: 10000)")
        modal.add_item(amt_input)
        
        async def on_modal_submit(intact: discord.Interaction):
            if not amt_input.value.isdigit(): return await intact.response.send_message("숫자만 입력 가능!", ephemeral=True)
            async with bot.db.acquire() as conn:
    ...
    async def info(self, interaction: discord.Interaction, button: Button):
        ...
        # 현재 등급 이름 찾기
        current_rank_name = "아이언"
        for amount, role_id in sorted(RANKS.items(), reverse=True):
            if spent >= amount:
                role = interaction.guild.get_role(role_id)
                current_rank_name = role.name if role else "알 수 없음"
                break

        embed = discord.Embed(title=f"👤 {interaction.user.display_name}님의 정보", color=discord.Color.blue())
        embed.add_field(name="🏆 현재 등급", value=f"**{current_rank_name}**", inline=True)
    ...
    async def calc(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("🧮 계산기 기능은 현재 고도화 작업 중입니다.", ephemeral=True)
