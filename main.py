...
class AdminVerifyApproveView(View):
    ...
    async def approve(self, interaction, btn):
        ...
        await interaction.response.send_message(f"✅ <@{self.target_id}> 님 인증 완료.", ephemeral=True)
...
class CarrierSelectView(View):
    ...
    async def mvno(self, it, b):
        ...
        for c in ["SKT 알뜰", "KT 알뜰", "LGU+ 알뜰"]:
            ...
            mv_view.add_item(btn)
        await it.response.edit_message(content="알뜰폰 통신사를 선택해주세요.", view=mv_view)
...
class DepositApproveView(View):
    ...
    async def reject(self, it, btn):
        ...
        await it.response.send_message("❌ 거부 처리됨.", ephemeral=True)
        await it.message.delete()
...
class ChargeModal(Modal):
    def __init__(self, bot):
        ...
        self.u_amount = TextInput(label="입금 금액", placeholder="숫자만 입력 (쉼표 제외)")
        self.add_item(self.u_sender); self.add_item(self.u_amount)

    async def on_submit(self, interaction):
        await interaction.response.defer(ephemeral=True)
        # 숫자만 추출하는 가장 확실한 방법
        raw_val = self.u_amount.value.replace(",", "").replace(" ", "").replace("원", "")
        if not raw_val.isdigit():
            return await interaction.followup.send("❌ 금액은 숫자만 입력해주세요 (예: 50000)", ephemeral=True)
        ...
        if log_ch:
            ...
            embed.add_field(name="금액", value=f"{amt:,}원", inline=False)
            await log_ch.send(embed=embed, view=DepositApproveView(req_id, interaction.user.id, amt, self.bot))
        
        await interaction.followup.send(f"✅ {amt:,}원 신청 완료! 성함 [ {sender} ](으)로 입금해주세요.", ephemeral=True)
...
class OTCView(View):
    ...
    async def check_verify(self, it):
        ...
        await it.response.send_message("🔒 본인인증 후 이용 가능합니다.", view=CarrierSelectView(self.bot), ephemeral=True)
        return False
    ...
    async def info(self, it, btn):
        ...
        rank_name = "일반"
        for val, r_id in sorted(RANKS.items(), reverse=True):
            if spent >= val:
